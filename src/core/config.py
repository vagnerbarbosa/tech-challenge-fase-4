...
class Settings(BaseSettings):
    ...
    azure_content_safety_key: str | None = Field(
        default=None,
        description="Chave da API Azure AI Content Safety",
        alias="AZURE_CONTENT_SAFETY_KEY",
    )
    azure_content_safety_endpoint: str | None = Field(
        default=None,
        description="URL do endpoint Azure AI Content Safety",
        alias="AZURE_CONTENT_SAFETY_ENDPOINT",
    )
    content_safety_enabled: bool = Field(
        default=False,
        description="Habilita Azure Content Safety para detecção multilíngue de risco",
        alias="CONTENT_SAFETY_ENABLED",
    )
    ...
    @model_validator(mode="after")
    def validate_content_safety_credentials(self) -> "Settings":
        """Valida se credenciais Content Safety estão configuradas quando habilitado."""
        if self.content_safety_enabled:
            missing = []
            if not self.azure_content_safety_key:
            ...
            if missing:
                raise ValueError(
                    f"Content Safety habilitado mas credenciais ausentes: {', '.join(missing)}"
                )
        return self
...
RISK_KEYWORDS: dict[str, list[str]] = {
    ...
    "saude_mental": [
        ...
        "miserável",
        "desgraça",
        # Ideação suicida - explícita e sutil
        "suicídio",
        "suicida",
        "morrer",
        "morra",
        ...
        "sem amigos",
        "sem amiga",
        # Comportamentos de risco e coping
        "beber",
        "bebi demais",
        "bebedo",
        "bêbada",
    ...
    # ===========================================
    # English Risk Keywords (for international users)
    # ===========================================
    "violence_en": [
        # Physical violence
        "violence",
    ...
    "mental_health_en": [
        ...
        "ptsd",
        "cptsd",
        # Substance use / Self-destructive
        "drinking too much",
        "drunk",
        "alcohol",
        "substances",
